...
local MissileBase = {Count=1,TrailColor=Color(210,210,210,200),TrailStartW=20,TrailLifeTime=1,TrailTexture="trails/smoke.vmt",BulletFunc=BulletFunc}
...
local DBase = {FireSound="weapons/rpg/rocketfire1.wav",shootfunc=Func,WireSpecial=WireSetup,WireFunc=WireFunc}


local function MergeData(D1,D2)
	local D = table.Copy(D1)
	table.Merge(D,D2)
	return D
end

--Basic missile luancher 4X
local Bullet=MergeData(Missiles["Basic"],MissileBase)
local Data={name="BML X4",class="bml_x4_weapon",In={"Missile Parts"},InUse={1},MountType="Medium",heat=5,firespeed=4,Points=800,Bullet=Bullet}
local Makeup = {name={"BML X4"},model={"models/slyfo/smlmissilepod.mdl"},Tool=Base.Tool,Type=Base.Type,class=Data.class}
LDE.Weapons.CompileWeapon(MergeData(DBase,Data),Makeup)

--Basic missile luancher 8X
local Bullet=MergeData(Missiles["Basic"],MissileBase)
local Data={name="BML X8",class="bml_x8_weapon",In={"Missile Parts"},InUse={1},MountType="Medium",MAO=Angle(0,0,90),heat=5,firespeed=2,Points=1200,Bullet=Bullet}
...
local Bullet=MergeData(Missiles["Basic"],MissileBase)
local Data={name="BML X10",class="bml_x10_weapon",In={"Missile Parts"},InUse={1},heat=5,firespeed=1,Points=1600,Bullet=Bullet}
...
local Bullet=MergeData(Missiles["Stinger"],MissileBase)
local Data={name="Stinger Launcher",class="stinger_missile_weapon",In={"Missile Parts"},heat=3,InUse={1},firespeed=3,Points=1600,Bullet=Bullet}
...
local Bullet=MergeData(Missiles["Heavy"],MissileBase)
local Data={name="Torpedo Launcher",class="heavy_missile_weapon",In={"Missile Parts","Liquid Polylodarium"},heat=50,InUse={3,10},firespeed=8,Points=2400,Bullet=Bullet}
...
--Nuclear luancher 1X
local Bullet=MergeData(MergeData(Missiles["Nuclear"],{ShootDir = Vector(0,-1,0),ShootPos=Vector(0,0,50)}),MissileBase)
local Data={name="Nuclear Launcher",class="nuclear_missile_weapon",In={"Missile Parts","Liquid Polylodarium"},heat=400,InUse={10,100},firespeed=1,Points=0,Bullet=Bullet}
